...
# Run
# Filter BAG_list to only those without existing plot files
BAG_list_to_run <- BAG_list[!sapply(BAG_list, function(bag) {
  plot_file <- file.path(output_dir, paste0("coffeechart_", trait, "_", bag, "_plot.rds"))
  file.exists(plot_file)
})]
# Run only for BAGs without existing plot files
all_results <- lapply(BAG_list_to_run, fit_and_test_effects)
# Assign names only for those actually run
names(all_results) <- BAG_list_to_run
