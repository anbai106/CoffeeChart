...
dietary_file="/Users/hao/cubic-home/Reproducibile_paper/CoffeeChart/data/dietary_data_encoded.tsv"
...
trait = 'average_weekly_red_wine_intake_f1568_0_0'
...
sleep <- read.csv(sleep_file)
names(sleep)[names(sleep) == "eid"] <- "participant_id"
sleep <- sleep[, c('participant_id', 'sleep_duration_f1160_0_0')]
...
df <- dietary %>%
  ...
  full_join(sleep, by = "participant_id") %>%
...
covariates <- c("age_when_attended_assessment_centre_f21003_0_0", "diastolic_blood_pressure_automated_reading_f4079_0_0",
                ...
                "smoking_status_f20116_0_0", 'average_total_household_income_before_tax_f738_0_0', 'sleep_duration_f1160_0_0', 'diagnosis')

# Function to test multiple families and return the best model
fit_and_test_effects <- function(outcome) {
  vars_to_keep <- c(trait, "sex_f31_0_0", "diagnosis", outcome, covariates)
  ...
  # Save the original outcome column before normalization
  df_model$BAG_original <- df_model[[outcome]]
  
  # Normalize the outcome for modeling
  df_model[[outcome]] <- (df_model[[outcome]] - min(df_model[[outcome]], na.rm = TRUE)) / 
    (max(df_model[[outcome]], na.rm = TRUE) - min(df_model[[outcome]], na.rm = TRUE)) + 1e-5
  
  # ---- New: define normative range from healthy controls ----
  hc <- df_model %>% filter(diagnosis == 0)
  if (nrow(hc) > 0) {
    norm_low <- quantile(hc$BAG_original, 0.25, na.rm = TRUE)
    norm_high <- quantile(hc$BAG_original, 0.75, na.rm = TRUE)
    
    min_val_orig <- min(df_model$BAG_original, na.rm = TRUE)
    max_val_orig <- max(df_model$BAG_original, na.rm = TRUE)
    norm_low_norm <- (norm_low - min_val_orig) / (max_val_orig - min_val_orig) + 1e-5
    norm_high_norm <- (norm_high - min_val_orig) / (max_val_orig - min_val_orig) + 1e-5
  } else {
    norm_low_norm <- NA
    norm_high_norm <- NA
  }
  ...
  # Significance threshold for adding lines
  add_optimal_lines <- main_dietary[["p-value"]] < 0.05 / 23
  optimals_filtered <- if (add_optimal_lines) optimals else optimals[0, ]
  
  df_model$red_wine_intake <- factor(df_model$average_weekly_red_wine_intake_f1568_0_0)
  
  p1 <- ggplot(df_model, aes(x = red_wine_intake, y = !!sym(outcome), fill = sex)) +
    geom_boxplot(position = position_dodge(width = 0.8), outlier.size = 0.5) +
    scale_fill_manual(values = c(female = "#EB5F2C", male = "#0072B5")) +
    labs(title = paste(outcome), x = "Red Wine intake", y = "BAG") +
    theme_minimal() +
    theme(legend.position = "none",
          axis.text = element_text(color = "black"))
  
  p2 <- ggplot(ci_data, aes(x = .data[[trait]], y = fit, color = sex)) +
  ...
  stats <- data.frame(
    ...
    Normative_low = norm_low_norm,
    Normative_high = norm_high_norm
  )
...
write.table(stats_table, 
            paste0(output_dir, "/BAG_RedWine_stats_GAM_CI_", trait, ".tsv"),
            sep = "\t", row.names = FALSE)